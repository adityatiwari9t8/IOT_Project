...
| `main.cpp` | `src/main.cpp` | Wires the above together. Reads a card, asks `StudentDatabase` what it means, tells `DisplayUI`/`FeedbackIndicators`/`AttendanceLogger` what to do. No pin numbers, no HTTP calls, no display primitives — just orchestration. |
...
## Why the roster resets every reboot

`Student.logged` lives in RAM, so a reset clears everyone back to "not
present." For a single Wokwi demo session that's the expected behavior —
you want a clean slate every time you restart the simulation. The Apps
Script's `isDuplicateToday_()` check exists specifically to cover the gap
this creates on real hardware (see README's "Known Limitations").
